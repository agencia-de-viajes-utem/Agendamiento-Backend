...
func ObtenerMasVistos(w http.ResponseWriter, r *http.Request) {
	...
	WITH ranked_packages AS (
		SELECT
			...
			fp.precio_oferta_vuelo AS oferta_vuelo,
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&paquetesMasVisto.FechaInit,
			&paquetesMasVisto.FechaFin,
			&paquetesMasVisto.NombreCiudadOrigen,
			&paquetesMasVisto.NombreCiudadDestino,
			&paquetesMasVisto.PrecioOfertaVuelo,
			...
			//&paquetesMasVisto.Aerolinea,
			//&paquetesMasVisto.AerolineaImagen,
		...
		fechaInicio, err := time.Parse("2006-01-02T15:04:05Z", paquetesMasVisto.FechaInit)
		if err != nil {
			log.Fatal(err)
		...
		fechaFin, err := time.Parse("2006-01-02T15:04:05Z", paquetesMasVisto.FechaFin)
		if err != nil {
			log.Fatal(err)
		...
		paquetesMasVisto.FechaInit = fechaInicio.Format("2006-01-02")
		paquetesMasVisto.FechaFin = fechaFin.Format("2006-01-02")
